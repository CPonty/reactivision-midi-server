...
#include "PortVideoSDL.h"
	...
	void CameraEngine::resetSettings() {
		//!TEST
		//stub to prevent linker complaints
		//when this virtual function isn't implemented in the OS-specific subclasses
	}
...
void CameraEngine::applyCameraSettingByte(int mode, int setting) {
	int max = getMaxCameraSetting(mode);
	int min = getMinCameraSetting(mode);
	int val = min+(int)(setting*((double)(max)-min)/127);
	switch (setting) {
		case 0: setCameraSetting(mode,min); break;
		case 127: setCameraSetting(mode,max); break;
		default: {
			if (setting>=0 && val<=setting) {
				setCameraSetting(mode,val);
			}
		}
	}
}
